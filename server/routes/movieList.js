...
router.get('/public', async (req, res) => {
  try {
    const lists = await MovieList.find({ isPublic: true }).populate('userId', 'username');
    res.json(lists);
  } catch (err) {
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    const list = await MovieList.findOneAndDelete({ _id: req.params.id, userId: req.user.id });
    if (!list) return res.status(404).json({ error: 'List not found' });
